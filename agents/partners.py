...
from agents.partner_artifacts import contract_call_artifact
from agents.partner_artifacts import filecoin_bundle_artifact
from agents.partner_artifacts import locus_payment_artifact
from agents.partner_artifacts import octant_signal_artifact
from agents.partner_artifacts import olas_marketplace_artifact
from agents.partner_artifacts import slice_hook_artifact
...
OFFLINE_EXECUTORS = {
    ...
    "hook_contract": slice_hook_artifact,
    "local_publish": octant_signal_artifact,
    "marketplace_bundle": olas_marketplace_artifact,
    "proof_bundle": filecoin_bundle_artifact,
    "signed_payment": locus_payment_artifact,
}
...
def build_http_request(
    ...
    """Return endpoint, headers, and request body for a live partner request."""
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "You are a private treasury risk analyst for public-goods "
                    "grants. Summarize safe next actions from the provided plan."
                ),
            },
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "Route the plan to the cheapest viable model family and "
                    "return a bounded execution recommendation."
                ),
            },
            {"role": "user", "content": json.dumps(payload, sort_keys=True)},
        ],
...
def handle_partner_error(
    ...
    """Convert known external service failures into structured execution results."""
    ...
    if requirement.name == "Bankr Gateway" and "does not have LLM Gateway access enabled" in detail:
        return {
