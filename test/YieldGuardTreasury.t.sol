...
contract YieldGuardTreasuryTest is Test {
    YieldGuardTreasury private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_yieldguard_opentrack");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new YieldGuardTreasury(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        YieldGuardTreasury freshController = new YieldGuardTreasury(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
